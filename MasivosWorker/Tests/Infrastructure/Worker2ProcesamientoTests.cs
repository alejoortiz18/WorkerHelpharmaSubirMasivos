...
public class Worker2ProcesamientoTests
{
    ...
    public async Task LoteFeliz_TresPdfProcesados_ActualizaLogYLimpiaTemporales()
    {
        ...
        var log = await File.ReadAllTextAsync(escenario.Contexto.RutaLogDiario);
        log.Should().Contain("CantidadProcesados:3");
        log.Should().Contain("NoProcesados:0");
    ...
    public async Task Tandas_SietePdf_ConTamanoLote3_ProcesaTodos()
    {
        ...
        var log = await File.ReadAllTextAsync(escenario.Contexto.RutaLogDiario);
        log.Should().Contain("CantidadProcesados:7");
    }
    ...
    public async Task Intento1_FalloBarcode_Intento2_Exito_Procesados()
    {
        ...
        documento.ProcesarAsync(Arg.Any<string>(), Arg.Any<CancellationToken>())
            .Returns(call =>
            {
                ...
                return ruta.Contains("error", StringComparison.OrdinalIgnoreCase)
                    ? Exito("KV111111")
                    : FalloBarcode();
            });
        ...
        await documento.Received(2).ProcesarAsync(Arg.Any<string>(), Arg.Any<CancellationToken>());

        var log = await File.ReadAllTextAsync(escenario.Contexto.RutaLogDiario);
        log.Should().Contain("CantidadProcesados:1");
    ...
    public async Task FalloApi_VaDirectoANoprocesados_SinPasarPorError()
    {
        ...
        var log = await File.ReadAllTextAsync(escenario.Contexto.RutaLogDiario);
        log.Should().Contain("NoProcesados:1");
        log.Should().Contain("CantidadProcesados:0");
    }
